...
import data_config
from datasets.CD_dataset import CDDataset ,ImageDataset
...
def get_loader(data_name, img_size=256, batch_size=8, split='test',
    ...
    elif dataset == 'CDDataset2':
        data_set = CDDataset2(root_dir=root_dir, split=split,
                                 img_size=img_size, is_train=is_train,
                                 label_transform=label_transform)
...
def get_loaders(args):
    ...
    if args.dataset == 'CDDataset':
        ...
        val_set = CDDataset(root_dir=root_dir, split=split_val,
                                 ...
                                 label_transform=label_transform)
    elif args.dataset == 'CDDataset2':
        training_set = CDDataset2(root_dir=root_dir, split=split,
                                 img_size=args.img_size,is_train=True,
                                 label_transform=label_transform)
        val_set = CDDataset2(root_dir=root_dir, split=split_val,
                                 img_size=args.img_size,is_train=False,
                                 label_transform=label_transform)
    else:
        raise NotImplementedError(
